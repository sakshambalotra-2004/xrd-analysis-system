Here is your exact original `README.md` with only the new pages, features (Multi-Scan Comparison, Polytype Support, OriginLab Export, and Gemini AI), and API routes surgically inserted. Nothing else has been changed!
...
- **Polytype Differentiation** — Specifically isolates and identifies exact crystalline polytypes (e.g., SiC 6H, 15R, 3C)
...
| 6 | Compound Identification | Predict compound name, formula, crystal system, and polytype |
...
| 10 | AI Insights | Generate automated executive summary via Gemini AI |
| 11 | Report Output | Render PDF and OriginLab `.opju` exports |
...
│   ├── public/
│   └── src/
│       ├── pages/                # Dashboard, Upload, Results, Analysis, Reports, ComparisonPage
│       ├── components/           # Navbar, XRDGraph, PeakTable, ConfidenceCard, MultiOverlayGraph, …
...
git clone [https://github.com/your-org/xrd-analysis-system.git](https://github.com/your-org/xrd-analysis-system.git)
...
5. To compare data, upload up to 3 files at once (or navigate to `/compare`) to automatically open the **Multi-Scan Comparison** dashboard.
6. Explore detailed crystallographic data on the **Analysis** page.
7. Download the full **PDF Report** or **OriginLab (.opju)** project from the Reports page.
...
| Report Generation | ReportLab / WeasyPrint, Origin Extensibility |

---

## Configuration

See [`config.py`](https://www.google.com/search?q=backend/config.py) for all configurable parameters including upload limits, peak detection thresholds, matching tolerance, wavelength, API Keys, and database paths.
